...
export default function InserimentoProdotto() {
  ...
  const [formData, setFormData] = useState({
    ...
    listino_tipo_id: 2,
    listino_modalita_id: "",
    listino_tipoCorso_id: 8, // LAUREE (Mantenuto di default)
    listino_durataLaurea_id: "",
    listino_facolta_id: "",
    listino_corsoLaurea_id: "",
    nome_universita_id: "",
    listino_attivoSN: -1, // -1 = Attivo, 0 = Non attivo
    listTesta_created_by: 1, // Verrà aggiornato dal localStorage se presente
  });

  // Stato per la griglia "Dettaglio prodotto" (array di righe)
  ...
  // Recupera l'utente dal localStorage e il prossimo codice disponibile all'avvio
  useEffect(() => {
    ...
    const fetchNextCode = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.codice) {
            setFormData((prev) => ({ ...prev, listTesta_codice: data.codice }));
          }
        }
      } catch (err) {
  ...
  // Funzione per rigenerare manualmente il codice su richiesta dell'utente
  ...
  // Gestione modifiche dentro la tabella dei dettagli
  ...
  // Aggiunge una nuova riga vuota nella tabella dei dettagli
  ...
  // Rimuove una riga dalla tabella dei dettagli
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepariamo il payload finale pulendo i campi vuoti per evitare errori 422
    const payload = {
      ...formData,
      listTesta_codice: formData.listTesta_codice || null,
    ...
    try {
      ...
      if (!response.ok) {
        console.error("Dettaglio errore 422 da FastAPI:", result);
        throw new Error(
          result.detail
