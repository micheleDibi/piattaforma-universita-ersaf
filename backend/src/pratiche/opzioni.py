...
def query_opzioni(db: Session, tipo: str, ricerca: str):
    if tipo == "studenti":
    ...
    query = query.filter(db.query(Pratica.pratica_id).filter(relazione).exists())
...
def opzioni(tipo: Literal["studenti", "percorsi"],
    ...
    righe = query_opzioni(db, tipo, ricerca.search).offset(ricerca.skip).limit(ricerca.limit + 1).all()
